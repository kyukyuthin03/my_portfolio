  ...
  return (
    <section id="education" className="border-b border-(--border) scroll-mt-20">
      <div className="mx-auto w-full max-w-6xl px-5 py-10 lg:px-8">
        <h2 className="mb-10 text-center text-3xl font-extrabold tracking-tight text-(--text)">Education</h2>
          {educationItems.map(item => (
            <article key={item.title} className="rounded-[1.75rem] p-6">
            <div className="flex items-center justify-between">
              <h3 className="mt-4 text-xl font-bold tracking-tight text-(--text)">{item.title}</h3>
              <p className="mt-4 text-xl font-bold tracking-tight text-(--text)">{item.gpa}</p>
            </div>
